 ...
 *
 * We could also produce Black and White TGA instead of color, this would 
 * require some bit buffer routines and some changes in the format header */
...
static header_t header(uint16_t width, uint16_t height) {
	static const header_t template = {
		.h = {
			0,          /* ID Length, 0 = No Id */
			0,          /* 0 = No color map */
...
static void reset(FILE *f) {
	assert(f);
	errno = 0;
	const int r = fseek(f, 0, SEEK_SET);
	if (r < 0)
		die("fseek failed on %p: %s", f, reason());
}

static inline size_t line_width(const char *s) {
	assert(s);
	return strlen(s) - 1;
}
...
static void write_pixel(FILE *out, char c) {
	assert(out);
	/*           Colors:        B    G    R    A    */
	unsigned char pixel[4] = { OFF, OFF, OFF, ON };
	if (c == ON_CHAR)
...
static int binary2tga(FILE *in, FILE *out) {
	...
	assert(out);
	uint16_t lc = 0;
	const uint16_t w = width(in);
	const uint16_t h = height(in);
	const header_t head = header(w, h);
...
int main(int argc, char **argv) {
	...
	FILE *in  = fopen_or_die(argv[1], "r");
	FILE *out = fopen_or_die(argv[2], "wb");
	binary2tga(in, out);
	fclose(in);
	fclose(out);
	return EXIT_SUCCESS;
